...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "item_type", sa.Text(), nullable=False, server_default=sa.text("'stop'")
        ),
        sa.Column("title", sa.Text(), nullable=False),
        ...
        sa.Column(
            "estimated_cost", sa.Float(), nullable=False, server_default=sa.text("0")
        ),
        sa.Column(
            "address_snapshot", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "reason_tags", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
        ...
        sa.ForeignKeyConstraint(
            ["linked_booking_id"], ["bookings.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
