<template>
  <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
    <div class="mb-8">
      <p class="text-sm font-semibold uppercase tracking-[0.25em] text-slate-500">Saved Places</p>
      <h1 class="mt-2 text-3xl font-bold text-slate-900">My Favourites</h1>
      <p class="mt-2 text-sm text-slate-500">All the listings you have saved for later.</p>
    </div>
    ...
    <div
      ...
      class="flex flex-col items-center justify-center rounded-3xl border border-slate-200 bg-white px-6 py-20 text-center shadow-sm"
    >
      ...
      <h2 class="mt-5 text-lg font-bold text-slate-900">No favourites yet</h2>
    ...
    >
      ...
      >
        <div class="relative overflow-hidden">
          <img
            v-if="favourite.listing?.image_urls?.length"
            :src="favourite.listing.image_urls[0]"
          ...
          <div class="absolute right-4 top-4">
            <button
              @click="removeFavourite(favourite.listing_id)"
        ...
        <div class="flex flex-1 flex-col p-6">
          <div class="mb-4">
            ...
            <div v-else-if="favourite.listing?.address" class="mt-3 flex items-start text-sm text-slate-500">
              ...
              <span class="line-clamp-2">{{ favourite.listing.address.city }}, {{ favourite.listing.address.country }}</span>
            </div>
          </div>

          <div v-if="favourite.listing?.avg_rating" class="mb-5 flex items-center justify-between">
            <div class="flex items-center gap-2">
              <div class="flex">
                <svg
                  ...
                  :key="i"
                  class="h-4 w-4"
                  :class="i <= Math.round(favourite.listing.avg_rating) ? 'text-amber-400' : 'text-slate-200'"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
              ...
              <span class="text-sm font-semibold text-slate-700">{{ favourite.listing.avg_rating.toFixed(1) }}</span>
            </div>
            <span v-if="favourite.listing?.review_count" class="text-sm font-medium text-cyan-700">
              {{ favourite.listing.review_count }} {{ favourite.listing.review_count === 1 ? 'Review' : 'Reviews' }}
            </span>
          </div>
          ...
          <div class="mt-auto flex items-center justify-between gap-4">
            <div>
              ...
              <p class="text-2xl font-bold text-slate-900">
                ${{ parseFloat(favourite.listing?.base_price || 0).toFixed(2) }}
              </p>
            </div>
            <router-link
              :to="`/listings/${favourite.listing_id}`"
...
async function removeFavourite(listingId) {
  try {
    await favouritesStore.remove(listingId)
  } catch (err) {
    console.error('Failed to remove favourite', err)
    toastStore.show('Failed to remove favourite.', 'error')
  }
}
</script>
