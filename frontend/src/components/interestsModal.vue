<template>
  ...
  >
    <div class="relative flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-white shadow-2xl transition-all duration-300">
      <div class="pointer-events-none absolute inset-x-0 top-0 h-32 bg-gradient-to-r from-cyan-100 via-white to-emerald-100"></div>
          ...
          <transition v-else :name="slideDirection" mode="out-in">
            <div :key="currentStep">
              <div class="mb-6 rounded-2xl border border-cyan-100 bg-gradient-to-r from-cyan-50 to-emerald-50 px-5 py-4">
                ...
                <h4 class="mt-1 text-xl font-bold capitalize text-slate-900">{{ categories[currentStep] }}</h4>
...
onMounted(async () => {
  try {
    ...
    toastStore.show('Failed to load interests.', 'error')
  } finally {
...
async function next() {
  ...
  try {
    ...
    authStore.setInterestsHandled()
    emit('interests-saved')
    emit('close')
  } catch (e) {
    console.error('Failed to save interests', e)
    toastStore.show('Failed to save interests.', 'error')
  } finally {
    saving.value = false
  }
}
...
async function confirmSkip() {
  ...
  try {
    ...
    toastStore.show('Failed to update your preference.', 'error')
  } finally {
    saving.value = false
  }
}
